...
export const UpdateDesignationLevels = () => {
    ...
    const getOneData = async () => {
        const { data } = await axiosInstance.get(`masterdesignations/view/${dataId}`)
        console.log(data[0]);
        console.log(data);
    ...
    const submit = async (e) => {
        ...
        try {
            console.log(data);
            if (!err) {
            const { result } = await axiosInstance.put(`masterdesignations/update/${dataId}`, ddata
            )
            swal("Yeah", "Designation Level Updated", "success");
