...
router.get('/view', async (req, res, next) => {
    ...
    let getQuery = `SELECT id, name FROM designation_levels where flag=1`;
    try {
...
router.get('/view/:id', async (req, res, next) => {
    console.log(req.params.id);
    let getQuery = `SELECT * FROM designation_levels where flag=1 and id=${req.params.id}`;
    try {
        let results = await pool.query(getQuery, (err, rows, fields) => {
            if (err) {
                res.send(err);
...
router.post('/', async (req, res, next) => {
    ...
    let insertQuery = `INSERT INTO designation_levels(name) VALUES ('${req.body.name}')`;
...
router.put('/update/:id', async (req, res, next) => {
    ...
    let insertQuery = `update designation_levels set name='${req.body.name}' where id=${req.params.id}`;
...
router.delete('/delete/:id', async (req, res, next) => {
    ...
    const myquery = `Delete from designation_levels WHERE id=${req.params.id}`
